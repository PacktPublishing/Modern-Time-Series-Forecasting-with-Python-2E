...
def make_stationary(
    x: np.ndarray, method: str = "detrend", detrend_kwargs: dict = {}
):
...
def forecast_bias_NIXTLA(
    ...
    res = (
        temp[models]
        .sub(
            temp[target_col], axis=0
        )  # Subtract the target column from the models predictions
        .div(
            _zero_to_nan(temp[target_col].abs()), axis=0
        )  # Normalize by the absolute values of the target column
        .fillna(0)
        * 100  # Fill NA with zero
    )
...
def forecast_bias(
    actual_series: Union[np.ndarray],
...
def metrics_adapter(
    ...
    """
    actual_series, pred_series = cast_to_series(actual_series), cast_to_series(
        pred_series
    )
    ...
    assert type(actual_series) is type(
        pred_series
    ), f"actual_series({type(actual_series)}) and pred_series({type(pred_series)}) should be of same type."
    if insample is not None:
        assert type(actual_series) is type(
            insample
        ), "actual_series and insample should be of same type."
    is_nd_array = isinstance(actual_series, np.ndarray)
    is_pd_series = isinstance(actual_series, pd.Series)

    if is_pd_series:
        is_datetime_index = is_datetime_dtypes(
            actual_series.index
        ) and is_datetime_dtypes(pred_series.index)
        if insample is not None:
            is_datetime_index = is_datetime_index and is_datetime_dtypes(
                insample.index
            )
    else:

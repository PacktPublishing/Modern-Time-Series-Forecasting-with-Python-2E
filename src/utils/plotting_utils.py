...
def multiple_line_plot_secondary_axis(
    df,
    x,
    primary,
    secondary,
    ...
    df = pd.pivot_table(
        df, index=x, columns=color_or_linetype, values=[primary, secondary]
    ).reset_index()
    df.columns = [
        str(c1) + "_" + str(c2) if c2 != "" else c1 for c1, c2 in df.columns
    ]
    ...
    for c, color in zip(primary_columns, colors[: len(primary_columns)]):
        fig.add_trace(
            go.Scatter(
                ...
                y=df[c],
                name=c,
                line=dict(dash=color) if use_linetype else dict(color=color),
            ),
            secondary_y=False,
        )
